 ...
 * @param {object} settings   Active profile settings (ragSeparator).
...
export function buildRagDocument(ragChunks, settings, charName) {
    ...
    const body = ragChunks.map(c => {
        ...
        return [sep, c.header, c.content].filter(Boolean).join('\n\n');
    }).join('\n\n***\n\n').trim();
 ...
 * @param {object} settings  Active profile settings (ragChunkSize).
 * @returns {Array}
 */

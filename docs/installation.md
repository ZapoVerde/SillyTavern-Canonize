...
4. Under **RAG Storage and Retrieval**, set your **Embedding Source** to **Voyage AI** (recommended). CNZ manages the Voyage AI key independently of SillyTavern — click the **API Key** button to set it. Voyage gives 200M free tokens per model: start with `voyage-4-large` and cascade to `voyage-4` when exhausted; by then `voyage-5` will likely be available.
